...
impl Calculator {
    ...
    pub fn show_history(&self) -> String{
        ...
        for (i,(op,result)) in self.history.iter().enumerate() {
            output.push_str(&format!("{}: {} {} {} = {}\n",i,op.first_num,op.operation_type.get_sign(),op.second_num,result));
        }
        return output;
    }
    ...
    // Return None if the index is invalid
     pub fn repeat(&mut self, operation_index: usize) -> Option<i64> {
        let (prev_op, _) = self.history.get(operation_index)?;
        let op = prev_op.clone();
